let model = {
  ...
  nouns: ['ebooks', 'websites', 'graphics', 'GIFs', 'buttons', 'web apps', 'risographs', 'videos', 'applescripts', 'printed matter', 'posters', 'python scripts', 'databases', 'tables', 'desks', 'shelves', 'beer', 'tattoos'],
...
let controller = {
  ...
  randomize: function(x) {
    let y = Math.floor(Math.random() * Math.floor(x.length));
    return x[y];
  },
  ...
  changeNouns: function() {
    // get random noun
    let tryNoun = controller.randomize(model.nouns);
    ...
    // get random color
    let tryColor = controller.randomize(model.colors);
